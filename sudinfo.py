...
class ConnectorDB:
    def __init__(self,root):
        self.root=root
        titlespace=""
        self.root.title(102*titlespace+"STUDENT INFO...")
        ...
        def iExit():
            iExit=tkinter.messagebox.askyesno("Mysqlf connection ","Confirm if you want to extit")
            if iExit>0:
                root.destroy()
                return
         #Reset button code   
        ...
        def addData():
            if  StudentID.get() == "" or Firstname.get()=="" or Surname.get() =="":
                tkinter.messagebox.showerror("Mysqlf connection ","Enter Correct details")
            else:
                 sqlCon=my.connect(host='localhost',user='root',passwd='',database='college')
                 cur=sqlCon.cursor()
                 cur.execute("insert into stud values(%s,%s,%s,%s,%s,%s)",(
                     StudentID.get(),
                     Firstname.get(),
                     Surname.get(),
                     Address.get(),
        ...
        def update():
            sqlCon=my.connect(host='localhost',user='root',passwd='',database='college')
            cur=sqlCon.cursor()
            cur.execute("update stud set firstname=%s,surname=%s,address=%s,gender=%s,mobile=%s where studid=%s",(
        ...
        def delete():
            sqlCon=my.connect(host='localhost',user='root',passwd='',database='college')
            cur=sqlCon.cursor()
            cur.execute("delete from stud where studid=%s"%StudentID.get())
        ...
        def search():
            try:
                sqlCon=my.connect(host='localhost',user='root',passwd='',database='college')
                cur=sqlCon.cursor()
                cur.execute("select * from stud where studid='%s'"%StudentID.get())
        ...
        self.lbltitle=Label(TitleFrame,font=('arial',40,'bold'),text="STUDENT INFO", bd=7)
        ...
        self.student_records=ttk.Treeview(LeftFrame, height=14,columns=("studid","firstname","surname","address","gender","mobile"),yscrollcommand=scroll_y.set)

        scroll_y.pack(side = RIGHT, fill=Y)

        self.student_records.heading("studid",text="StudentID")
        ...
        self.student_records.bind("<ButtonRelease-1>",collegeInfo)
        #DispalyData()
        #***************************************************************************************      
        ...
        self.btnUpadate = Button(RightFrame1a,font=('arial',16,'bold'),text="Update",bd=4,pady=1, padx=24, width=8,height=2,command=update).grid(row=2,column=0,padx=1)
